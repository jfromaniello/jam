...
exports.updateRequireConfig = function (package_dir, baseurl, callback) {
    ...
    exports.getAllPackages(package_dir, function (err, pkgs) {
        ...
        try{
            var extraConfig = require(path.join(package_dir, 'require-extra.json'));
            Object.keys(extraConfig.shim || {}).forEach(function(k){
                shims[k] = extraConfig.shim[k];
            });
            Object.keys(extraConfig.map || {}).forEach(function(k){
                map[k] = extraConfig.map[k];
            });
            (extraConfig.packages || []).forEach(function(extraPkg){
                packages.push(extraPkg);
            });
        }catch(e){}
